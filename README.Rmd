...
The goal of pkgDataTest is to illustrate the issue I'm having with a minimal reproducible expample. The issue is that I have created an internal function called fn_docalc in which I reference a package-exported dataset called measurements.  I expect that fn_docalc will always operate on this package-version of the measurements variable.  However, in the case that the user also creates a global variable with the same name as the dataset (measurements), then fn_docalc is using that new variable to operate on instead.

## Installation

You can install the development version of pkgDataTest from [GitHub](https://github.com/) with:
